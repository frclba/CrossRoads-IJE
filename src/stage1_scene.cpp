...
void Stage1Scene::bullet(){
  if(player_controller->attack_ranged && timestep < Game::instance.timer->getTicks()){
...
bool Stage1Scene::is_inside(GameObject* object){
  if(object->state() == GameObject::State::enabled&&(object->main_positionX > 0 || object->main_positionX + object->main_width < 800)){
